...
module.exports = {
  async find (ctx) {
    ...
    try {
      ...
      ctx.response.message = 'find medicines failed'
    }
  },
  ...
  async findOne (ctx) {
    ...
    try {
      const value = await strapi.query('medicines').model.findOne({
        _id: ObjectId(_id),
        health_id: ObjectId(health_id)
      })
  ...
  async update (ctx) {
    ...
    try {
      const value = await strapi.query('medicines').model.findOneAndUpdate({
        health_id: ObjectId(health_id),
      ...
      if (!value) {
        ctx.response.status = 400
        ctx.response.message = 'medicine not found'
        return
      }
